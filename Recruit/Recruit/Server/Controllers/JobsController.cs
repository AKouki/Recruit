...
namespace Recruit.Server.Controllers
{
    ...
    public class JobsController : ControllerBase
    {
        ...
        [HttpGet("Due")]
        public IEnumerable<Job> GetDueJobs()
        {
            var jobs = _db.Jobs.OrderBy(j => j.Expires).Take(10).ToList();
            return jobs;
        }
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            var model = new JobDetailsModel
            {
                JobId = job.Id,
                JobTitle = job.Title,
                Applicants = job.Applicants ?? Enumerable.Empty<Applicant>(),
                Stages = job.Stages ?? Enumerable.Empty<Stage>()
            };
        ...
        {
            var jobToEdit = _db.Jobs.Find(id);
            if (jobToEdit == null)
                return NotFound();
